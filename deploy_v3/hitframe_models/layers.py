...
class CoordinateEmbedding(nn.Module):
    ...
    def forward(self, x):
        # x shape: (batch, seq_len, 2, 17, 2) -> reshape to (batch, seq_len, 2, 34)
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2], -1)
        p1 = x.select(2, 0)  # Player 1 keypoints: (batch, seq_len, 34)
        p2 = x.select(2, 1)  # Player 2 keypoints: (batch, seq_len, 34)
        ...
        projected = torch.cat((p1, p2), 2)  # (batch, seq_len, emb_size)
        return projected
